                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="border rounded-lg divide-y">
                        {isLoading ? (
                            ...
                            <div className="p-4 text-center text-muted-foreground">No tracks added yet.</div>
                        ) : (
                            tracks.map((track) => (
                                <div key={track.id} className="flex items-center justify-between p-3 hover:bg-muted/50 transition-colors">
            ...
            <Dialog open={!!trackToDelete} onOpenChange={(open) => !open && setTrackToDelete(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            This action cannot be undone. To confirm, please type the track ID below.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-2">
                        <div className="text-sm font-medium">
                            Track Name: <span className="font-normal">{trackToDelete?.name}</span>
                        </div>
                        <div className="bg-muted p-2 rounded text-sm font-mono break-all">
                            {trackToDelete?.id} {/* will remove as soon as the testing of this thing is done */}
                        </div>
                        <div className="space-y-2">
                            <label className="text-sm font-medium">Type Track ID to confirm:</label>
                            <Input
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setTrackToDelete(null)} disabled={isDeleting}>
                            Cancel
                        </Button>
                        <Button
                            variant="destructive"
